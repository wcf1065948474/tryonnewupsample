...
class BilinearSamplingBlock(nn.Module):
    ...
    def forward(self, source, flow_field):
        ...
        warp = torch.nn.functional.grid_sample(source, grid,align_corners=True)    
        return warp    
...
class ResBlockDecoder(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, hidden_nc=None, norm_layer=nn.BatchNorm2d, nonlinearity= nn.LeakyReLU(),
        ...
        bypass = spectral_norm(nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=1, padding=1), use_spect)

        if type(norm_layer) == type(None):
            self.model = nn.Sequential(nonlinearity, conv1, nonlinearity,conv2_up, conv2,)
        else:
            self.model = nn.Sequential(norm_layer(input_nc), nonlinearity, conv1, norm_layer(hidden_nc), nonlinearity,conv2_up, conv2,)
